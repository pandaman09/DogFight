...
function GM:GetTeamScoreInfo()
	...
	for id,pl in pairs( player.GetAll() ) do
		...
		local _team = pl:Team()
		local _frags = pl:GetNWInt("money", 0)
		local kl = pl:GetNWInt("kills", 0) 
		local dt = pl:GetNWInt("deaths",0)
		local _deaths = pl:CalcKD()
		...
		if (not TeamInfo[_team]) then
			...
			TeamInfo[_team].Score = team.GetScore(_team)
		end		
...
function GM:HUDDrawScoreBoard()
	...
	surface.SetTextPos( xOffset + boardWidth - (colWidth*2) + 8,	y)	surface.DrawText("K:D")
