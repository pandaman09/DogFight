...
function GM:OnRoundStart( num )
	...
	for k,v in pairs(player.GetAll()) do
		v:SetMainScore(0)
		v:PrintCenter("Fuel will spawn in "..GAMEMODE.FuelSpawnDelay.." seconds!",3)
	end
	for k,v in pairs(ents.FindByClass("df_fuel_node")) do
		v:OnRoundStart()
	end
	DF_FIRST_BLOOD = true
end
...
function GM:OnRoundEnd( num )	
	--GAMEMODE:GetRocket(TEAM_RED).dt.Fuel = 0
	GAMEMODE:GetRocket(TEAM_RED):SetDTInt("Fuel", 0)
	GAMEMODE:GetRocket(TEAM_BLUE):SetDTInt("Fuel", 0)
	for k,v in pairs(player.GetAll()) do
		v:KillSilent()
	end
end
...
function GM:OnPreRoundStart( num )
	...
	for k,v in pairs(ents.FindByClass("df_rocket")) do
		v:Reset() // in case it launched
	end
	self.RoundPostLength = 20
end
...
hook.Add( "PlayerDisconnect", "RoundCheck_PlayerDisconnect", function() timer.Simple( 0.2, function() GAMEMODE:CheckPlayerDeathRoundEnd() end ) end )
...
function GM:CheckRoundEnd()
	...
	if !IsValid(self:GetRocket(TEAM_RED)) or !IsValid(self:GetRocket(TEAM_BLUE)) then
		return false --temp prevent round from ending
	end
	
	--if GAMEMODE:GetRocket(TEAM_RED).dt.Fuel >= GAMEMODE.MaxFuel  then
	if self:GetRocket(TEAM_RED):GetDTInt("Fuel") >= self.MaxFuel  then
		self:RoundEndWithResult(TEAM_RED, "Team red fueled their rocket!")
		self:GetRocket(TEAM_RED):Launch()
		...
		umsg.Start("df_startrocketcam")
		umsg.Short(TEAM_RED)
		umsg.Vector(cam_pos)
		umsg.End()
	end
	--if GAMEMODE:GetRocket(TEAM_BLUE).dt.Fuel >= GAMEMODE.MaxFuel  then
	if self:GetRocket(TEAM_BLUE):GetDTInt("Fuel") >= self.MaxFuel  then
	
		self:RoundEndWithResult(TEAM_BLUE, "Team blue fueled their rocket!")
		self:GetRocket(TEAM_BLUE):Launch()
		local cam_pos = Vector(0,0,0)
		for k,v in pairs(ents.FindByClass("df_fuel_node")) do
...
function GM:RoundTimerEnd()
	...
	if ( !GAMEMODE.TeamBased ) then
		...
		GAMEMODE.RoundPostLength = 5 //If Time runs out we dont want to watch a rocket go off.	
		GAMEMODE:RoundEndWithResult( -1, "Time Up. You're all losers!" )
	end
