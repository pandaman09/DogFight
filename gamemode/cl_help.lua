...
function GM:ShowHelp()
	...
	if ( !IsValid( Help ) ) then
		...
		if ( IsValid( LocalPlayer() ) ) then
			...
			local btn = Help:AddSelectButton("Settings", function() GAMEMODE:ShowSettings() end)
			btn.m_colBackground = Color(255,180,0)
			
			local btn = Help:AddSelectButton("Leaderboards", function() GAMEMODE:ShowLeaderBoard() end)
			btn.m_colBackground = Color(255,100,100)
